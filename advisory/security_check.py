...
from db import DB_PATH, ENCRYPTION_KEY_ENV, ensure_token_crypto_ready
...
def run_security_check() -> dict[str, object]:
    ...
    if DB_PATH.exists():
        ...
        if stat.S_IMODE(DB_PATH.stat().st_mode) & 0o077:
            fail("db_file_permissions", f"DB permissions are {mode}; expected 0o600.", severity="medium")
        else:
            ok("db_file_permissions", f"DB permissions are {mode}.")
    else:
        checks.append({"check": "db_file_permissions", "status": "info", "detail": "DB file not created yet."})
